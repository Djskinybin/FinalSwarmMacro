...
; ============================================================
; Everything lives in the same FinalSwarmMacro folder:
;
;   FinalSwarmMacro\
;       Loader.ahk
;       FinalSwarmMacro.ahk
...
;       settings\
;
; No Icons folder is used or required.
; ============================================================

assetRoot := A_ScriptDir "\"

use720Mode := (A_ScreenWidth = 1280 && A_ScreenHeight = 720)

upgradeFolder := assetRoot "upgrades\"
weaponFolder := assetRoot "Weapons\"

; Always use the normal downloaded source images.
; At 1280x720, ImageSearch scales the card dimensions as needed.
native720Images := false

if (
    !DirExist(upgradeFolder)
    || !DirExist(weaponFolder)
) {
    MsgBox(
        "FinalSwarmMacro could not find its downloaded image folders.`n`n"
        . "Expected layout:`n"
        . A_ScriptDir "\Loader.ahk`n"
        ...
        . "Run Loader.ahk again to redownload the macro and PNGs.",
        "FinalSwarmMacro - Assets Not Found",
        "Iconx"
    )
    ExitApp
}
...
settingsFolder := assetRoot "settings\"
settingsFile := settingsFolder "settings.ini"

restartFolder := assetRoot "restart\"
...
EnsureSettingsFolder() {
    global settingsFolder

    if DirExist(settingsFolder)
        return true

    try DirCreate settingsFolder

    if !DirExist(settingsFolder) {
        MsgBox(
            "FinalSwarmMacro cannot access its settings folder.`n`n"
            . "Expected:`n"
            . settingsFolder
            . "`n`n"
            . "Make sure FinalSwarmMacro was extracted to a normal folder "
            . "and is not being run directly from inside a ZIP/RAR archive.",
            "FinalSwarmMacro - Settings Folder Error",
            "Iconx"
        )
        return false
    }
...
SafeIniWrite(value, filename, section, key) {
    ...
    try {
        ...
        MsgBox(
            "FinalSwarmMacro could not save its settings.`n`n"
            . e.Message
            . "`n`nPath:`n"
            . filename,
            "FinalSwarmMacro - Settings Save Error",
            "Iconx"
        )
        return false
    }
...
imageModeText := farmGui.AddText(
    "xm y+" GS(5) " w" GS(180),
    (use720Mode ? "Image Set: scaled for 1280x720" : "Image Set: 1920x1080")
)
...
SaveUpgradeLimits() {
    ...
    for name, limit in upgradeMaxSelections
        SafeIniWrite limit, settingsFile, "UpgradeLimits", name
}
...
SaveAppSettings(*) {
    ...
    SafeIniWrite(closeScriptHotkey, settingsFile, "Interface", "CloseHotkey")
    SafeIniWrite(pauseScriptHotkey, settingsFile, "Interface", "PauseHotkey")

    if farmPaused
        SetPauseHotkeyEnabled(false)
    ...
    SafeIniWrite(rerollsPerLife, settingsFile, "Actions", "RerollsPerLife")
    SafeIniWrite(banishesPerLife, settingsFile, "Actions", "BanishesPerLife")
    ...
    if newDarkMode != darkModeEnabled {
        ...
        SafeIniWrite(darkModeEnabled ? "1" : "0", settingsFile, "Interface", "DarkMode")
        ApplyThemeToAllGuis()
    }
    else {
        SafeIniWrite(darkModeEnabled ? "1" : "0", settingsFile, "Interface", "DarkMode")
...
SetPauseHotkeyEnabled(enabled) {

    global pauseScriptHotkey

    if pauseScriptHotkey = ""
        return
    ...
    try {
        if enabled
            ...
            Hotkey registeredHotkey, PauseFarm, "Off"
    }
}
...
PauseFarm(*) {
    ...
    ; Release the pause key while paused so it works normally for typing.
    SetPauseHotkeyEnabled(false)
...
ResumeFarmFromPause(*) {
    ...
    ; Restore the pause keybind for the next pause.
    SetPauseHotkeyEnabled(true)
...
SaveSelections(*) {
    ...
    SafeIniWrite islandDropdown.Text, settingsFile, "Game", "Island"
    SafeIniWrite challengeDropdown.Text, settingsFile, "Game", "Challenge"
    SafeIniWrite difficultyDropdown.Text, settingsFile, "Game", "Difficulty"
    SafeIniWrite defaultToleranceDropdown.Text, settingsFile, "Game", "DefaultTolerance"
}
...
SavePriorityEditor(*) {
    ...
    SafeIniWrite ArrayToString(commonUpgrades), settingsFile, "Priorities", "Common"
    SafeIniWrite ArrayToString(rareUpgrades), settingsFile, "Priorities", "Rare"
    SafeIniWrite ArrayToString(epicUpgrades), settingsFile, "Priorities", "Epic"
    SafeIniWrite ArrayToString(legendaryUpgrades), settingsFile, "Priorities", "Legendary"
    SafeIniWrite ArrayToString(weaponPriority), settingsFile, "Priorities", "Weapons"
    SafeIniWrite (rerollCommonEnabled ? "1" : "0"), settingsFile, "RerollByRarity", "Common"
    SafeIniWrite (rerollRareEnabled ? "1" : "0"), settingsFile, "RerollByRarity", "Rare"
    SafeIniWrite (rerollEpicEnabled ? "1" : "0"), settingsFile, "RerollByRarity", "Epic"
    SafeIniWrite (rerollLegendaryEnabled ? "1" : "0"), settingsFile, "RerollByRarity", "Legendary"
